#setwd("/Users/zhaoyikai/Comparative_pol_measurement_project/After_dissertation/wave6/Shiny_both_waves/")
...
dta = read.csv("nonzero_ds_wv5.csv")
dta6 = read.csv("nonzero_ds_wv6.csv")

mutual_country = intersect(unique(dta$country), unique(dta6$country))



plot_n = function(dta, num_cluster, wv){
  country_name = dta[[2]]
  dta = dta[[1]]
...
ui <- fluidPage(
  ...
  titlePanel(
      "Visualization of 'Democracy' variables - Comparison between wave 5 & 6" 
  ),
  
  br(),
  fluidRow(#largest fluidRow
    column(width = 6, offeset = 2, selectInput(inputId = "COUNTRY",
                       label = "Country to visualize",
                       choices = unique(mutual_country)),
      ...
      fluidRow( # second fluidRow
               tabsetPanel(
                 tabPanel("Profile plot",
                          fluidRow(
                            
                            column(width = 6,
                                   plotOutput("NUM_N_PLOT5")
                            ),
                            
                            column(width = 6,
                                   plotOutput("NUM_N_PLOT6")
                            ),
                          ), #closing for fluidRow
                          
                          p(("The plotting may take 10-20 secs, please wait & do not change tabs to avoid 
          unneccesary computation lags :)" ), style= 'padding:30px;font-size:100%')),
                 
                 tabPanel("3-D plot", 
                          fluidRow(
                            
                            column(width = 6,
                                   plotOutput("THREE_D_PLOT5")
                            ),
                            
                            column(width = 6,
                                   plotOutput("THREE_D_PLOT6")
                            ),
                          ), height = "auto"),
                 ...
                 tabPanel(paste("Profile plot", "of the best model -- DO NOT RANDOMLY CLICK, OTHERWISE NEED 30s to RUN!", sep = "\n"), 
                          fluidRow(
                            ...
                            column(width = 6,
                                   plotOutput("BEST_PLOT5")
                            ),
                            
                            column(width = 6,
                                   plotOutput("BEST_PLOT6")
                            ),
                          ), height = "auto",
      ...
      ) #closing for the second fluidRow
  ...
  ) # closing for the largest fluidRow
) #closing for the fluidPage 
...
server <- function(input, output){
  DATA5 = reactive(list({dta %>%
      filter(country == input$COUNTRY) %>%
      dplyr::select(c("tax", "religion", "free_election", "state_aid", "Army",
                      "civil_rights", "women"))}, input$COUNTRY))
  
  DATA6 = reactive(list({dta6 %>%
      filter(country == input$COUNTRY) %>%
      dplyr::select(c("tax", "religion", "free_election", "state_aid", "Army",
                      "civil_rights", "women"))}, input$COUNTRY))
  
  f5 = reactive({with(DATA5()[[1]], cbind(tax, religion, free_election, state_aid, Army, civil_rights, women)~1)}) 
  f6 = reactive({with(DATA6()[[1]], cbind(tax, religion, free_election, state_aid, Army, civil_rights, women)~1)}) 
  ...
  output$NUM_N_PLOT5 = 
      renderPlot({
        plot_n(DATA5(), input$NUM, 5)})
    
  output$NUM_N_PLOT6 = 
    renderPlot({
      plot_n(DATA6(), input$NUM, 6)})
    
    

  output$THREE_D_PLOT5 = renderPlot({
             p = poLCA(f5(), DATA5()[[1]], nclass= input$NUM, maxiter=3000, 
                       tol=1e-5, na.rm=FALSE,  
                       nrep=10, verbose=TRUE, calc.se=FALSE, graphs = TRUE)
             print(p)
           }, height = function(){300+100*input$NUM})

  output$THREE_D_PLOT6 = renderPlot({
    p = poLCA(f6(), DATA6()[[1]], nclass= input$NUM, maxiter=3000, 
              tol=1e-5, na.rm=FALSE,  
              nrep=10, verbose=TRUE, calc.se=FALSE, graphs = TRUE)
    print(p)
  }, height = function(){300+100*input$NUM})
 ...
 # closing for fluidRow
  ...
  output$BEST_PLOT5 = renderPlot({
             best_plot(DATA5())})    

    
  output$BEST_PLOT6 = renderPlot({
    best_plot(DATA6())})   
